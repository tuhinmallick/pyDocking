...
class BranchedNet(torch.nn.Module):
    ...
    def __init__(self, input_size_1, input_size_2):
        ...
        self.conv11 = nn.Conv2d(
            input_size_1[0], 128, kernel_size=self.kernel, stride=self.stride, padding=self.padding)
        self.channel = 128
        ...
        self.conv21 = nn.Conv2d(
            input_size_2[0], 128, kernel_size=self.kernel, stride=self.stride, padding=self.padding)
        self.channel = 128
        self.w = int((self.w - self.kernel + 2 *
                     self.padding) / self.stride + 1)
        self.h = int((self.h - self.kernel + 2 *
